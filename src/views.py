...
class ApartmentViewQueries(APIView):
    ...
    @classmethod
    def get_active_program_appartments(self):
        return Apartment.objects.filter(program__is_active=True)

    @classmethod
    def get_programs_price_between(self):
    ...
    def list_appartments_by_season(self):
        ...
        if current_month in [6, 7, 8, 9]:
            return Apartment.objects.annotate(
                custom_order=Case(
                    When(caracteristics__icontains="swimming pool", then=Value(1)),
                    default=Value(2),
            ...
            return Apartment.objects.annotate(
                custom_order=Case(
                    When(caracteristics__icontains="near ski resort", then=Value(1)),
                    default=Value(2),
                    output_field=IntegerField(),
                )
            ).order_by('custom_order', '-price', '-surface')
...
class ApartmentPromoCode(APIView):
    serializer_class = ApartmentPromoSerializer
    @classmethod
    def list_program_promo_code(self,promo_code):
